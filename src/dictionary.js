// @source: https://alcapones.com/slang_dictionary.php
...
const db_entries = Object.entries(db);

const fetch = function(count) {
  let a = [];
  for(let i=0; i<count; i++) {
    const r = Math.floor(Math.random() * db_entries.length);
    let slice = db_entries.slice(r, r+1).map(e => {
      return {
        phrase: e[0],
        tip: e[1],
      };
